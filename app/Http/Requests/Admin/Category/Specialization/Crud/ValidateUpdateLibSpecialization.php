...
class ValidateUpdateLibSpecialization extends FormRequest
{
   /**
    ...
    public function rules($request,$row): array
    {
        ...
        if($row->isDirty('name')) {
            $rules['name'] = [
                ...
                Rule::unique('lib_specializations')->where(fn($q) => $q->where('lib_subcategory_id', $subcategoryId))->ignore($row->id),
            ];
        }
