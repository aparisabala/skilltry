$(document).ready(function(){
    ...
    if ($('#frmStoreAdminUserRole').length > 0) {
        ...
        PX.ajaxRequest({
            ...
            validation: true,
            script: 'admin/hrm/user/user-role',
            rules,
            afterSuccess: {
    ...
    if ($('#frmUpdateAdminUserRole').length > 0) {
        ...
        PX.ajaxRequest({
            ...
            validation: true,
            script: 'admin/hrm/user/user-role/'+$("#patch_id").val(),
            rules,
            afterSuccess: {
    ...
    if ($("#dtAdminUserRole").length > 0) {
        ...
        let col_draft = [
...

            {
                ...
                render: function (data, type, row) {
                    let str = ``;
                    if(policy?.hrm_user_roles_edit) {
                        str += `<a href="${baseurl}admin/hrm/user/user-role/${data.id}/edit" class="btn btn-outline-secondary btn-sm edit" title="Edit">
        ...
        PX.renderDataTable('dtAdminUserRole', {
            select: true,
            url: 'admin/hrm/user/user-role/list',
            columns: col_draft,
            pdf: [1, 2]
        });
    }
...
function dtAdminUserRole(table, api, op) {
    PX.deleteAll({
        ...
        script: "admin/hrm/user/user-role/delete-list",
        confirm: true,
        api,
    });
    PX.updateAll({
        element: "updateAllAdminUserRole",
        script: "admin/hrm/user/user-role/update-list",
        confirm: true,
        dataCols: {
            key: "ids",
            items: [
