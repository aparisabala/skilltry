        ...
        <div class="col-md-12">
            @can('ac_ledger_crud_view')
                @if($data['item'] != null || $data['type'] != "edit")
                    <div class="">
                        @include('admin.pages.account.ledger.crud.fragments._breadcum')

                        <div class="page-block-body">
                            @if($data['type'] == "add")
                                @include('admin.pages.account.ledger.crud.fragments._actions')
                                <div class="card rounded page-block">
                                    ...
                                    <div id="defaultPage" class="table-list pages">
                                        <div class="mt-2 p-2 p-md-4">
                                            ...
                                            <input type="hidden" id="ledger_type" value="{{ $data['ledger_type'] }}" />

                                            @if(count($data['items']) > 0)
                                                @include('common.view.fragments._show-selected')
                                                @include('common.view.fragments._pdf-layout',['docTitle' => 'AcLedger List'])
                                                <div class="table-responsive">
                                                    <table class="table table-bordered dt-responsive nowrap dataTable no-footer dtr-inline collapsed" id="dtAcLedger"></table>
                                                </div>
                                            @else
                                    ...
                                    <div id="addPage" class="d-none pages">
                                        @include('admin.pages.account.ledger.crud.fragments._add')
                                    </div>
                                </div>
                            @else
                                <div id="editPage" class="pages">
                                    <div class="card rounded-0 pb-3">
                                        <div id="loadEdit" class="w-100">
                                            @include('admin.pages.account.ledger.crud.fragments._edit')
                                        </div>
                                    </div>
                                </div>
                            @endif
