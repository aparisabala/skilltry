<div class="bg-info pl-2 page-fragment-bar">
    <span class="text-light"> <a href="{{url('admin/datalibrary/location/thana')}}"><span class="badge badge-info cursor-pointer"> <i class='fa-solid fa-arrow-left fs-16'></i></span></a> <span class="pt-1"> {{pxLang($data['lang'],'update')}}   </span> </span>
</div>
<div class="mt-4 p-3">
    @can('lib_thana_edit')
        <form id="frmUpdateLibThana" autocomplete="off">
            @method('PATCH')
                                    ...
                                    <div class="input-group">
                                        <select class="form-control" name="lib_division_id" id="lib_division_id">
                                            ...
                                            @foreach($data['divisions'] as $item)
                                                <option value="{{$item->id}}" {{($data['item']?->lib_division_id == $item->id) ? 'selected' : ''}}>{{$item->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="input-group">
                                        <select class="form-control" name="lib_district_id" id="lib_district_id">
                                            ...
                                            @foreach($data['districts'] as $item)
                                                <option value="{{$item->id}}" data-division="{{$item->lib_division_id}}" {{($data['item']?->lib_district_id == $item->id) ? 'selected' : ''}}>{{$item->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
